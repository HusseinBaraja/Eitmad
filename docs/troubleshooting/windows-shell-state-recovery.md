...
keywords:
  ...
  - "+ إضافة تصنيف جديد missing"
  - "+ إضافة وحدة جديدة missing"
  - "new category not selected"
  - "archived unit still selectable"
  - "PartsView popup clipped"
  - "الأجزاء لا تظهر"
  - "Wardrobe Side Panel missing"
  - "YER cost appears reversed"
---
...
- **+ إضافة تصنيف جديد**, **+ إضافة وحدة جديدة**, **إدارة التصنيفات**, or **إدارة الوحدات** is missing from an editor selector; saving a new reference does not select it; or an archived reference remains selectable.
...
| A **المواد الخام** dropdown looks like an old square Windows control, or its row-action popup opens on the wrong RTL side | A raw-material selector uses the platform-default template, or popup placement and Arabic text share one direction boundary | Inspect `RawMaterialsView.xaml` for `RawMaterialsComboBox`, `PART_Popup`, `RawMaterialsContextMenu`, and RTL `RawMaterialsMenuItem`; verify the popup placement boundary is LTR while Arabic items remain RTL | Restore the raw-material control templates and direction isolation. Run `RawMaterialsRenderedTests.RowActionPopupUsesItsTargetAndNonDestructiveActions`, then inspect an open category selector and three-dot action popup in the rendered WPF window |
| The **الأجزاء** action popup is clipped at the left edge, or Edit cannot be clicked | WPF combines the RTL table boundary with a right-edge placement mode for the physical-left action column | Inspect `PartsView.xaml` for `PartsContextMenu`, `FlowDirection="LeftToRight"`, and `Placement="MousePoint"`; reopen the menu from **إجراءات الجزء** | Keep mouse-point placement for the Parts menu, rebuild, and verify **تعديل**, **تكرار**, and **أرشفة** are inside the window and clickable |
| **الأجزاء** shows no rows, the **Wardrobe Side Panel** amount is missing, or **3 Products** is reordered | `PartsViewModel` is not bound, the mixed-direction fixture was changed, or cost/usage cells lost their LTR boundary | Run `PartsSearchAndFiltersUpdateVisibleList`, `PartsActionsRemainNonDestructiveAndEphemeral`, and `PartsRenderedTests`; inspect `PartListItem` cost and usage projections | Restore `VisibleParts`, `CostAmountLabel`, `CurrencyLabel`, and `UsedInLabel` bindings. Keep the synthetic fixture and close the preview to roll back local edits |
| Inline category or unit actions are missing, a new reference is not selected, or an archived reference stays selectable | The editor selector lost its `category`/`unit` tag, bypasses `ActiveCategories` or `ActiveUnits`, or updates only the visible label instead of `RawMaterialsViewModel` state | Inspect the tagged editor selectors, `RawMaterialReferenceOption`, and the `BeginAdd*`, `SaveReferenceEditor`, and `ArchiveReference` paths; then run `RawMaterialReferencesCanBeManagedInline` | Restore the shared popup footer and ViewModel lifecycle. Keep filters untagged, add valid references to the correct active collection, set `EditorCategory` or `EditorUnit` after add, and remove archived references only from the active selector collection. Do not add shell persistence or delete the reference record |
...
| The **حفظ في المعاينة** label is dark, or the **إلغاء** button has a missing right border | `PrimaryButton` does not forward `Foreground` to its `ContentPresenter`, or the secondary button border is not pixel-snapped and is too faint at the rounded edge | Inspect `OperationsTheme.xaml` for a `TextElement.Foreground` binding to the templated button foreground; inspect `RawMaterialsSecondaryButton` for its explicit border brush and `SnapsToDevicePixels` | Restore the shared content foreground binding, the pixel-snapped secondary border, and inspect both editor actions in the rendered WPF window |
