...
| Handshake authentication rejection | The inherited bootstrap token is absent, malformed, or does not match this engine generation | Confirm that the supervisor launched the current engine and connected with its retained token | Stop the failed generation and let the supervisor start a new paired process; do not copy or override the token |
...
| Handshake rejects missing `eitmad.capability.authorization-scopes.v1` | Peer cannot preserve protocol `1.3` tenant/workspace context | Compare generated binding and offered/required capability sets | Upgrade the shell/adapter with the engine; never disable scoped authorization |
...
Do not delete runtime files, expose bootstrap or bearer tokens, copy raw frames, or include customer records, paths, authorization graphs, or process environment values in support evidence.

## Verify recovery

Confirm a new engine generation reaches `Ready`, the handshake reports the expected version within `1.0–1.6`, and `eitmad.capability.authorization-scopes.v1` plus operation-specific capabilities are present. Confirm that Rust returns the tenant, scope, principal, and session; the shell must not send these fields. Verify one synthetic scoped subscription receives an ordered event and a revoked synthetic member receives no later configuration event. Intentional shutdown must reach `Stopping → Stopped` with exit code `0`.
