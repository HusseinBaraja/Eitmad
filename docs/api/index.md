...
The encoded window is `1.0–1.3`. Protocol `1.0` supports command/query traffic. Protocol `1.1` adds `eitmad.capability.local-ipc-subscriptions.v1`. Protocol `1.2` adds relationship mutations/listing, authorization-policy events, and `authorizationRevoked`. Protocol `1.3` adds decodable tenant/workspace fields but requires a non-nil tenant plus `eitmad.capability.authorization-scopes.v1`; local IPC rejects missing context or capability before normal traffic. Policy events still require `eitmad.capability.authorization-policy-events.v1`. See the [protocol 1.3 release](../releases/protocol-1-3-scoped-authorization-audit.md) for coordinated binding rollout.
