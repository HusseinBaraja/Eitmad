...
The server creates the next named-pipe instance before serving an accepted connection, so reconnecting clients do not encounter a missing-listener gap. It dispatches independent commands and queries concurrently and returns responses by `RequestId`; callers must not infer completion order. Each connection accepts at most 64 in-flight requests and owns at most 64 active subscriptions. At the request limit, Rust stops reading until one request completes, which bounds task and frame growth without rejecting already-read work. It returns `eitmad.error.ipc-subscription-capacity-exceeded.v1` before creating broker state or a pump when the subscription ceiling is reached. One background Windows reader routes responses and events, buffers an event that races its subscribe acknowledgement, and rejects events for unknown subscriptions. The Windows codec reads event dictionaries and payload elements directly, so it does not serialize an event a second time during dispatch.
