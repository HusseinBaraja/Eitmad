...
description: "Understand the Arabic-first Furniture list and its transient Information, Parts, and fixed-Variants preview steps."
audience: "developer"
...
The Windows **الأثاث** page gives managers a compact product table and the first three steps of a six-step furniture editor. The page is a local preview. Search, filters, image selection, part quantities, calculated values, variants, duplicate, and archive state are discarded when the shell closes.

## Ownership and current boundary

`shells/windows/Features/Furniture/FurnitureView.xaml` owns the native RTL layout, small vector thumbnails, upload preview surface, custom selectors, tables, overlays, keyboard focus targets, and Arabic accessibility names. `FurnitureViewModel.cs` owns transient fixtures, list filtering, the current editor step, selected parts, preview totals, and fixed-variant cards. `FurnitureModels.cs` owns presentation-only row labels and calculated display values. `MainWindow.xaml` owns the **الأثاث** sidebar destination.
...
## Three implemented editor steps

The header shows the complete future sequence: **المعلومات**, **الأجزاء**, **المقاسات**, **الخيارات**, **التسعير**, and **المراجعة**. Only the first three steps are implemented:
...
The **التالي** action on step 3 stays on the current screen and reports that **الخيارات** is not implemented. This stop is deliberate. Do not add Options, Pricing, Review, or a final save command as part of this preview.
...
`FurniturePresentationTests` verifies Arabic search normalization, composed state changes, part totals, fixed variants, and the deliberate stop at step 3. `FurnitureRenderedTests` instantiates the real WPF window, navigates to **الأثاث**, verifies Arabic accessible names and keyboard focus, selects a part, adds a variant, and checks the physical row-popup placement. A manual rendered check at `1338 × 753` verifies the compact manager table, large image preview, part-cost summary, and side-by-side variant cards.
