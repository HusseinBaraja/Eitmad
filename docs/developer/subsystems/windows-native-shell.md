...
The separate **التسعير** destination is a quick manager preview under `Features/Pricing`. `PricingViewModel` owns only transient product-and-variant fixtures, Arabic-normalized search, category filtering, and the small **تعديل سعر البيع** interaction. The RTL table begins with **المنتج** at the physical right edge and makes **سعر البيع** the strongest value; `YER` amounts use local LTR boundaries. The modal shows product, fixed variant, cost, selling-price input, and the calculated absolute margin. It never shows raw materials or parts and never opens the Furniture editor. Save updates the selected in-memory fixture and reports **حُدث سعر البيع في المعاينة المحلية فقط.** No Rust pricing command, authorization decision, audit record, durable storage, or synchronization exists, so production saving must remain unavailable until the owning Rust vertical and typed contract are implemented.

The **عروض الأسعار** destination is a manager review preview under `Features/Quotations`. `QuotationsViewModel` owns only synthetic quotation rows, Arabic-normalized number and customer search, status and relative-date filters, selected detail state, and temporary discount decisions. The list shows **رقم عرض السعر**, **العميل**, **التاريخ**, **الإجمالي**, **الخصم**, **الحالة**, and **فتح**. It groups the visible states as **مسودة**, **نشط**, **محوّل**, and **ملغي / منتهي**. The detail is read-only: it shows quotation metadata, furniture lines with variant, color, handle, quantity, unit price, and total, followed by subtotal, discount, and final total. The **موافقة الخصم مطلوبة** section appears only for a fixture marked as requiring manager review; **موافقة** and **رفض** record a local preview decision and do not authorize or mutate a quotation. A production implementation must add a Rust quotation vertical, typed contracts, relationship-based manager authorization, scoped atomic approval and audit, durable storage, idempotency, and synchronization before either action can report an authoritative result.

The **الطلبات** destination is a manager review preview under `Features/Orders`. `OrdersViewModel` owns synthetic order rows, Arabic-normalized order-number and customer search, status and relative-date filters, selected detail state, and calculated totals. The list shows **رقم الطلب**, **العميل**, **التاريخ**, **الإجمالي**, **الحالة**, and **فتح**. It exposes the provisional fixture states **جديد**, **قيد الإنتاج**, **جاهز**, **تم التسليم**, and **ملغي**. The read-only detail shows order metadata plus each product, variant, dimensions, color, handle, quantity, and selling price, followed by subtotal, discount, and final total. It has no status-changing action and no carpenter workflow; the latter belongs to the separate work-order capability. A production implementation must add a Rust order vertical, typed contracts, relationship-based manager authorization, scoped mutation audit, durable storage, idempotency, and synchronization before the preview can show authoritative records.

The **أوامر العمل** destination is a manager manufacturing preview under `Features/WorkOrders`. `WorkOrdersViewModel` owns synthetic work-order rows, Arabic-normalized search across the work-order number, related Order, Customer, Furniture, and assigned carpenter, plus status and due-date filters. One Work order can contain multiple Furniture items. The scan-first list shows **رقم أمر العمل**, **العميل / الطلب**, an item summary under **الأثاث**, total unit **الكمية**, **مسند إلى**, **موعد التسليم**, **الحالة**, and **فتح**. Its four provisional states are **جديد**, **قيد التنفيذ**, **مكتمل**, and **ملغي**. Opening a row shows the compact related Order, Customer, carpenter, and due date followed by a count and one illustrated specification card for every Furniture item, with fixed dimensions, color, handle, and item quantity. The page then shows the required Parts and prominent **ملاحظات الطلب**. Selling price, cost, profit, and margin are absent. The one status action advances **جديد** → **قيد التنفيذ** → **مكتمل** and explicitly reports that the change is local preview state. Ready-made Products never enter this list. A production implementation must add a Rust work-order vertical, typed contracts, manager authorization, explicit record scope, atomic status mutation and audit, durable storage, idempotency, and synchronization before any status change can report an authoritative result.
