...
The receptionist **المنتجات** destination opens one visual sales catalog under `Features/Reception`. It reads sales-only projections from the existing Furniture and Products preview models each time it opens, independent of manager list filters. Cards combine published furniture and active ready-made products without an internal type label. They show an in-memory image or synthetic vector illustration, name, short description, size or variant summary, and the lowest variant selling price. Cost, margin, materials, parts, and internal notes are absent. The page uses existing manager categories without category editing. Arabic-normalized search matches name or category and composes with the selected category. **لا توجد منتجات مطابقة** provides **مسح الفلاتر**, which clears both filters and returns focus to search. **اختيار** opens furniture selection with a large existing image or vector illustration, read-only size cards, active manager-defined colors and handles, and a reusable `QuantityStepper`. No custom dimensions, colors, or handles are accepted. Options start unselected; the add action is enabled only after all available sections have a selection. Furniture with no sizes cannot be added. The preview updates the base price, option additions, unit price, and quantity total immediately. **إضافة إلى عرض السعر** snapshots a temporary line and shows a preview success notice without leaving the catalog. The existing receptionist header action shows the line count and opens the temporary quotation only on request. Returning to the catalog preserves its filters and returns keyboard focus to the selected card. Catalog illustrations are shared through `Resources/SalesCatalogVisuals.xaml`; selection cards reuse `FilterChip` without changing other pages. Ready-made selection uses variant cards when applicable, then quantity; it has no furniture options. The Home **عرض سعر جديد** action opens the catalog and focuses search without requiring customer details. It preserves temporary quotation lines when returning from Home. Review opens only through an explicit quotation action; continuing restores selection or catalog focus. All selection prices and quotation lines are synthetic, in-memory presentation state; no quotation is saved or synchronized. Production queries, authorization, audit, and storage remain Rust-owned. Verify this surface with `SalesCatalogPresentationTests` and `SalesCatalogRenderedTests`.
