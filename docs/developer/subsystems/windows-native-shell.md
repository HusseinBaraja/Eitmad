...
The **أوامر العمل** destination is a manager manufacturing preview under `Features/WorkOrders`. `WorkOrdersViewModel` owns synthetic work-order rows, Arabic-normalized search across the work-order number, related Order, Customer, Furniture, and assigned carpenter, plus status and due-date filters. The scan-first list shows **رقم أمر العمل**, **العميل / الطلب**, **الأثاث**, **الكمية**, **مسند إلى**, **موعد التسليم**, **الحالة**, and **فتح**. Its four provisional states are **جديد**, **قيد التنفيذ**, **مكتمل**, and **ملغي**. Opening a row shows the compact related Order, Customer, carpenter, and due date followed by an illustrated Furniture specification, fixed dimensions, color, handle, quantity, required Parts, and prominent **ملاحظات الطلب**. Selling price, cost, profit, and margin are absent. The one status action advances **جديد** → **قيد التنفيذ** → **مكتمل** and explicitly reports that the change is local preview state. Ready-made Products never enter this list. A production implementation must add a Rust work-order vertical, typed contracts, manager authorization, explicit record scope, atomic status mutation and audit, durable storage, idempotency, and synchronization before any status change can report an authoritative result.
