...
The in-memory and persisted engines accept at most 2,048 pending changes and 2,048 pending commands per scope. New work fails with `QueueFull` after idempotent replay checks. This bound prevents disconnected work from causing unlimited memory, database, or network growth.
