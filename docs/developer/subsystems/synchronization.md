...
| Direct authenticated HTTPS/WebSocket connection and Rust-owned credential lifecycle | `crates/server-connection/src/lib.rs` |
...
For the direct route, Rust stores the access token, rotating refresh token, expiry times, account/device identity, and Ed25519 signing seed as one replaceable `SecretMaterial` under a typed `SecretId`. The route requires an HTTPS endpoint and an explicit PEM trust anchor. Rustls verifies the certificate chain, server name, and validity; failed verification returns `EncryptionRequired` before HTTP or sync traffic. Rust signs a new device proof for the WebSocket upgrade and for token refresh. Refresh occurs before access expiry, replaces the stored token pair, and deletes the unusable stored session after a rejected refresh or expired refresh token. If access expires during a live WebSocket session, the server reports `server-token-expired`; the client reconnects and refreshes before establishing another session. Other authentication failures remain terminal. The native shell receives only connection state and structured errors.

The driver bounds TLS connection and I/O waits to five seconds, WebSocket messages to 1 MiB, refresh responses to 16 KiB, and pull requests to the protocol batch maximum. It permits one outstanding pull at a time because the current server response is a `ServerMessage::Sync` without a stream identifier. The driver maps that response to the outstanding frame's stream and correlation; a snapshot maps to consecutive frames and ends on completion. Cancellation sends the existing cancel frame. If the bounded response is already in flight, the driver drains and discards its remaining messages before accepting another request. The server checks the negotiated protocol on each frame and caps inbound WebSocket messages at 1 MiB. The shared transport core owns reconnect and exponential backoff; the driver does not add another retry loop.
...
The in-memory and persisted engines accept at most 2,048 pending changes and 2,048 pending commands per scope. New work fails with `QueueFull` after idempotent replay checks. State version `2` preserves an oversized queue loaded from legacy version `1` only so the engine can drain existing work; it does not admit more work, and it removes the legacy marker after both queues return within bounds. This bound prevents disconnected work from causing unlimited memory, database, or network growth.

The reference-marker vertical demonstrates local-first integration without a second sync protocol. Its state transaction writes one registered `eitmad.schema.reference-marker.v1` change to a durable outbox. Background sync reads at most 50 changes, sends the shared change records, and confirms an exact `change_id`. A confirmation cannot hide a newer pending edit to the same marker.
...
Related references: [sync contracts](../../api/synchronization-contracts.md), [reference-marker vertical](reference-marker.md), [server authority](server-authority.md), [WAN relay](wan-relay-coordination.md), [local storage](local-storage.md), [authorization](authorization.md), [ADR-0008](../../decisions/0008-required-unified-synchronization.md), [storage version 8 rollout](../../releases/storage-v8-reference-marker.md), and [protocol 1.5 rollout](../../releases/protocol-1-5-operational-server-planes.md).
