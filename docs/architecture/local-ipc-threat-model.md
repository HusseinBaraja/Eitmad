...
| Assert another tenant or role | The handshake returns a device principal without the installation owner relation. Protocol 1.7 sign-in resolves a provisioned account in Rust and returns its distinct user principal | Account provisioning and control-plane synchronization remain trusted Rust operations |
...
| Protocol downgrade or drift | Mandatory `PeerHello`, generated bindings, and a protocol 1.7 gate on desktop session operations. Older peers cannot dispatch product work | Old shell versions need an update before authenticated desktop work |
