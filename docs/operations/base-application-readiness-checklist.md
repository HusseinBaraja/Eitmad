...
- **Pass:** local IPC uses protocol 1.7, a private process bootstrap token, a separate Rust-authenticated business user session, exact session context, bounded frames, bounded concurrency, and capability negotiation.
...
- **Pass:** local storage version 10 has immutable checksummed migrations, pre-migration backups, drift checks, integrity checks, restore rollback, and recovery documentation.
