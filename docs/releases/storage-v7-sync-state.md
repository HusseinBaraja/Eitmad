...
Storage version 7 adds `sync.scoped-state.v1`: one scope-bound durable `sync_scopes` row containing mode-specific records, snapshots, pending work, conflicts, idempotency replays, processed deliveries, and metadata. Rust-generated protocol artifacts now include `eitmad.sync.reconcile.v1` and the expanded sync contract graph.
...
- Migration 7 creates `sync_scopes`; it does not infer an application mode or synthesize domain records. A row is created when `SyncEngine::open` first configures a scope.
...
Generated sync shapes changed with this foundation implementation. Deploy the Rust engine, JSON schema, C#/Swift bindings, identifier constants, and compatible peers together. Complete protocol/capability/schema negotiation before sync traffic.
...
4. Configure each domain scope exactly once as local-first or server-authoritative. Do not select mode in a shell preference.
