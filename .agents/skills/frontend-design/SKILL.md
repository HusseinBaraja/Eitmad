...
## Codex project adaptation

Adapted on 2026-09-13 from https://github.com/anthropics/skills/tree/main/skills/frontend-design. The upstream LICENSE.txt is retained. This section replaces the upstream opening guidance.

Use this skill for Eitmad native Arabic UI design. Apply AGENTS.md, native WPF behavior, existing Arabic typography, and theme resources before the web examples below. Inspect and reuse suitable shared controls, configuring them locally without changing other pages. Create a reusable shared control only if none fits.

Use the supplied brief and repository as design context. Ask only about missing decisions that materially affect the result. Keep planning proportional: identify the primary task, existing color and type tokens, and native RTL layout. An established visual system does not need new tokens or written wireframes. Do not add decorative heroes, motion, or a new framework to an operational list.

Keep account authority in Rust. Label fixtures and temporary changes as preview-only. Inspect the affected rendered screen with available WPF checks and stop after the focused proof passes.

Make deliberate, restrained visual choices for daily workshop operations.
