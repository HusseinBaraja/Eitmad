...
5. A user follows English help for an Arabic-first workflow without needing implementation details.
...
docs/
  ...
  user/                       # English help for Arabic-first product workflows
    index.md
  ...
  releases/                   # compatibility, migration, and upgrade notes
    index.md
  glossary.md                 # English definitions with canonical Arabic UI terms
...
- Write all repository documentation prose in English, including user, developer, operations, API, architecture, troubleshooting, decision, and release pages.
- Treat Arabic as a product UI/UX requirement, not the documentation language. Include Arabic only when recording exact UI labels, approved product terms, search aliases, localized symptoms, or representative RTL and bidirectional examples.
- Keep Rust names, commands, paths, error IDs, and configuration keys exact and LTR.
- Define each product term once in the English glossary and pair it with the canonical Arabic UI term. Include local workshop synonyms only as search aliases unless approved as canonical.
